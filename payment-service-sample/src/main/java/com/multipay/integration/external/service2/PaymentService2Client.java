/**
 * This package is part of integration layer for sample external payment service provider 2
 */
...
import com.multipay.integration.external.service2.dto.ExternalService2PaymentRequestDTO;
import com.multipay.integration.external.service2.dto.ExternalService2PaymentResponseDTO;
import com.multipay.utils.MessageEnums;

/**
 * 
 * Class responsible to communicate with external payment service provider 2
 * 
...
public class PaymentService2Client {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PaymentService2Client.class);
	...
	public ProcessPaymentResponseDTO processPayment(ProcessPaymentRequestDTO paymentRequest) throws TechnicalException {
		...
		ResponseEntity<ExternalService2PaymentResponseDTO> responseEntity = null;
		ProcessPaymentResponseDTO response;
		ExternalService2PaymentRequestDTO externalRequest = prepareExternalRequest(paymentRequest);
		try {
	 ...
	 * 
	 */
	private ExternalService2PaymentRequestDTO prepareExternalRequest(ProcessPaymentRequestDTO paymentRequest) {

		ExternalService2PaymentRequestDTO externalRequest = new ExternalService2PaymentRequestDTO();
		externalRequest.setAmount(paymentRequest.getAmount());
		externalRequest.setPaymentDate(null);
	 ...
	 * 
	 */
	private ResponseEntity<ExternalService2PaymentResponseDTO> sendPaymentRequestToExternalService(ExternalService2PaymentRequestDTO externalRequest)
		...
		if (LOGGER.isDebugEnabled()) {
			...
			buffer.append("\n\t").append("Request send to external Paymenr service 1: ->").append("\n\t")
					...
					.append("To url: -> ").append(configParameters.getPaymentservice2url());
		...
		try {
			...
			ResponseEntity<ExternalService2PaymentResponseDTO> externalResponse = restTemplate
					.postForEntity(configParameters.getPaymentservice2url(), externalRequest, ExternalService2PaymentResponseDTO.class);
			if (LOGGER.isDebugEnabled()) {
				...
				bufferResponse.append("MultiPay ExternalService1 -> sendPaymentRequestToExternalService response.")
						...
						.append("External Service2 response :").append(externalResponse.getBody()).append("\n\t");
	 ...
	 * 
	 */
	private ProcessPaymentResponseDTO processExternalResponse(ResponseEntity<ExternalService2PaymentResponseDTO> responseEntity,
		...
		if (ObjectUtils.isNotEmpty(responseEntity) && ObjectUtils.isNotEmpty(responseEntity.getBody())) {
			...
			ExternalService2PaymentResponseDTO externalResponse = responseEntity.getBody();
			if (LOGGER.isDebugEnabled()) {
				String jsonResponse = objectMapper.writeValueAsString(externalResponse);
				StringBuffer buffer = new StringBuffer();

				buffer.append("\n\t").append("Response received from external payment service 2: ->").append("\n\t")
			...
			if (responseEntity.getStatusCode() == HttpStatus.OK && externalResponse.getStatusCode().equals("00")) {
				header.setSuccessful(true);
			} else {
				LOGGER.error("Payment response received indicates the failure of payment: ");
				header.setDescription(externalResponse.getStatusMessage());
				header.setCode(externalResponse.getStatusCode());
